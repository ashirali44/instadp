...
class Request {
    ...
    static requestTransform(body, response, resolveWithFullResponse) {
        if (response.headers['content-encoding'] === 'zstd') {
            try {
                body = Buffer.from((0, fzstd_1.decompress)(Buffer.from(response.rawBody))).toString();
            }
            catch (e) { }
        }
        try {
    ...
    updateState(response) {
        const { 'x-ig-set-www-claim': wwwClaim, 'ig-set-authorization': auth, 'ig-set-password-encryption-key-id': pwKeyId, 'ig-set-password-encryption-pub-key': pwPubKey, 'ig-set-ig-u-ds-user-id': UserID, 'ig-set-x-mid': xMid, 'ig-set-ig-u-rur': Rur, 'ig-set-ig-u-shbts': shbts, 'ig-set-ig-u-ig-direct-region-hint': Region, } = response.headers;
